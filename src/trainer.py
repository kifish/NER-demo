...
class Trainer_v2:
    def __init__(self, config):
        ...
        self.criterion = self.model.batch_neg_log_likelihood
    ...
    @staticmethod
    def __acc(pred, label, skipped_class = [0, 7]):
        # pred: 2-d list; label: 2-d tensor
        # 0是padding; 7是O
    ...
    def train(self, train_data_loader, val_data_loader):
        ...
        for epoch in tqdm(range(self.config.num_epoch)):
            ...
            for batch_idx, batch_samples in enumerate(tqdm(train_data_loader)):
                ...
                ############# train  model #####################
                self.optimizer.zero_grad()
                loss = self.criterion(input_ids, attention_mask, target_ids)
                loss.backward()
                ...
                loss_total += loss.item() 
                train_loss = loss_total / n_step # train_mean_seq_loss in the epoch; crf算token loss没意义
                
                self.writer.add_scalar('Train/loss', train_loss , global_step)

                if global_step % self.config.print_every == 0:
                    print_cnt += 1 
                    # 与Trainer不同: 参数更新完之后进行的预测
                    
                    b_score, b_tag_seq = self.model.batch_forward(input_ids, attention_mask)
                    b_raw_acc, b_target_acc, b_pure_target_acc, b_real_padding_ratio, b_pred_padding_ratio = self.__acc(b_tag_seq, target_ids)
                    # 如果最后一个batch的bsz变小, 结果会有出入
                    raw_acc_total += b_raw_acc
                    target_acc_total += b_target_acc
                    pure_target_acc_total += b_pure_target_acc
                    ...
                    # train_loss实际上是上一个iteration的loss
                    self.config.logger.info(
                        'epoch {}, iteration {}, '
                        'train_mean_seq_loss: {:.4f}, '
                        'train_mean_raw_acc: {:.4f}, '
    ...
    def inference(self, data_loader, mode = 'val'):
        ...
        mean_seq_loss = loss_total / n_batch
