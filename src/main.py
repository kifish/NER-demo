...
if use_pretrain_embedding:
    ...
    model = BiLSTM_crf(num_labels = 8,word_vocab_size = vocab_size, max_seq_len = transformer_x.max_len,use_crf = True)
...
if use_pretrain_embedding:
    if not os.path.exists('../data/pretrain_embedding'):
        os.mkdir('../data/pretrain_embedding')
    save_path = '../data/pretrain_embedding/pred.txt'
else:
    if not os.path.exists('../data/no_pretrain_embedding'):
        os.mkdir('../data/no_pretrain_embedding')
    save_path = '../data/no_pretrain_embedding/pred.txt'
