...
class BertCRFForNER_v2(BertPreTrainedModel):
    def __init__(self, model_config, **kwargs):
        bert_config = BertConfig.from_pretrained(model_config['base_model_name'], cache_dir = model_config['cache_dir'])
        super(BertCRFForNER_v2, self).__init__(bert_config)
...
# very fast
# 目前不支持多卡并行
class BertCRFForNER_v3(BertPreTrainedModel):
    def __init__(self, model_config, **kwargs):
        bert_config = BertConfig.from_pretrained(model_config['base_model_name'], cache_dir = model_config['cache_dir'])
        super(BertCRFForNER_v3, self).__init__(bert_config)
        ...
        # CRF这样实现比较优雅, 巧妙地解决了start tag和end tag的问题
    ...
    def batch_forward(self, input_ids, attention_mask = None): 
        ...
        # 会把input padding部分（即SEP之后的部分）截断
