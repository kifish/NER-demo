...
class BiLSTM_cnn_crf(object):
    def __init__(self,
                 ...
                 filters_num = 30,
                 fc_dim=100,
                 dropout=0.5, # maybe we could change 0.5 to 0.2
                 embedding_matrix=None,
                 use_char=True,
                 use_crf=True,
                 optimizer = 'adam'):
        """build a Bi-LSTM-cnn-crf model.
        Args:
            word_vocab_size (int): word vocabulary size.
            char_vocab_size (int): character vocabulary size.
            num_labels (int): number of entity labels.
    ...
    def build(self,metrics = ['accuracy']):
        ...
        padding_layer = ZeroPadding1D(padding= half_window_size,name='zero_padding1D')(z)
        # ZeroPadding1D不支持mask_zero
        # Layer conv1d_1 does not support masking
        conv = Conv1D(filters = self.filters_num,kernel_size = 2 * half_window_size + 1, \
                       padding = 'valid',activation= 'tanh')(padding_layer) # 思考是否要加tanh
        conv_d = Dropout(0.05,name='conv_dropout')(conv)
        dense_conv = TimeDistributed(Dense(30),name= 'time_distributed_dense_conv')(conv_d)
        lstm_cnn_merge = Concatenate(name = 'merge_cnn')([z,dense_conv])
        fc = TimeDistributed(Dense(self.fc_dim, activation= 'tanh',name= 'time_distributed_fc_layer'))(lstm_cnn_merge)
        #如果在TensorFlow里面还需要把z flat之后再进fc_layer
        ## applies fully-connected operation at every timestep
        # fc = Dense(self.fc_dim, activation='tanh', name='time_distributed_fc_layer')(z)
        # 没有TimeDistributed 没法配合crf
...
if __name__ == '__main__':
    # model = BiLSTM_crf(num_labels= 8, word_vocab_size= 20000, max_seq_len = 100)
    # model = model.build()
    # model.summary()
    model = BiLSTM_cnn_crf(num_labels= 8, word_vocab_size= 20000, max_seq_len = 100)
    model = model.build()
    model.summary()
